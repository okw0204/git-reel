...
- README、`.env.example`、`AGENTS.md` の説明を新しい接続方針に合わせる。
...
`.env.example` は OAuth 関連と公開 URL だけを残す。

`AGENTS.md` は実装と同じコミット範囲で最新化する。実行時の注意から `GITHUB_TOKEN` fallback、`/api/auth/dev-connect`、通常 seed fallback の説明を削除し、ローカル開発でも GitHub OAuth 設定が必要であることを書く。構成メモでは、空のリール候補は `DiscoveryService::ensure_candidates` が保存済み OAuth token だけで補充を試すこと、seed はテスト用の候補準備に限定すること、E2E が dev-connect 前提ではなくなることを明記する。
...
- `AGENTS.md` が OAuth 必須、`GITHUB_TOKEN` 削除、dev-connect 削除、seed テスト専用の方針に更新されている。
