...
- `GITHUB_TOKEN` を設定すると、候補が空のときに GitHub Search API から最近更新されたリポジトリを補充する。取得に失敗した場合やトークン未設定時は開発用シード候補にフォールバックする。
...
- 空のリール候補は `DiscoveryService::ensure_candidates` が `GITHUB_TOKEN` 設定時に実 GitHub Search API を試し、候補が採用できなければ固定の開発用候補を投入する。候補追加や重複排除の挙動変更は `server/src/discovery.rs`、`server/src/github.rs`、repository store を確認する。
